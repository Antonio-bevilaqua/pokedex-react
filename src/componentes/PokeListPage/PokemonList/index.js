...
function PokemonList() {
  ...
  const { pokemons, setListScroll, fetchNewPage, opacity } =
  ...
  const handleScroll = async (event) => {
    ...
    if (actualScrollPosition >= target.scrollHeight - 250) {
      ...
      const scrollNext =
        pokemonsRef.current[nextIndex].offsetTop - listRef.current.clientHeight;
      listRef.current.scrollTop = scrollNext;
    }
  };

  const scrollList = (actualScrollPosition) => {
    const { i1, i2, actualOpacity } =
      findLastTwoPokemonCardProperties(actualScrollPosition);
